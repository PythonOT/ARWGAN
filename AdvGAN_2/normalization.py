...
class WeightNorm_todel(Wrapper):
    ...
    def __init__(self, layer, data_init=False, **kwargs):
        ...
        super(WeightNorm_todel, self).__init__(layer, **kwargs)
    ...
    def build(self, input_shape):
        ...
        super(WeightNorm_todel, self).build()
        self.built = True
...
def Layernorm(name, norm_axes, inputs):
    mean, var = tf.nn.moments(x=inputs, axes=norm_axes, keepdims=True)
    ...
    offset = tf.compat.v1.get_variable("offset", [n_neurons], initializer=tf.constant_initializer(0.0, dtype=tf.float32))
    scale = tf.compat.v1.get_variable("scale", [n_neurons], initializer=tf.constant_initializer(0.0, dtype=tf.float32))
