...
def make_simpler_conv_net(nb_class=10):
    def simple_conv_net(x, scope_name='classifier_conv_net', reuse=False):
        with tf.compat.v1.variable_scope(scope_name, reuse) as scope:
            if reuse:
                scope.reuse_variables()

            net = layers.conv2d(x, 3, 64, 2, name='conv0_1', batch_normed=False)
            net = tf.nn.relu(net)
            net = layers.conv2d(net, 3, 128, 1, name='conv1_1', batch_normed=False)
            net = tf.nn.relu(net)
            net = layers.conv2d(net, 3, 256, 1, name='conv2_1', batch_normed=False)
            net = tf.nn.relu(net)
...
def make_simple_conv_net(nb_class=10):
    def simple_conv_net(x, scope_name='classifier_conv_net', reuse=False):
        with tf.compat.v1.variable_scope(scope_name, reuse) as scope:
            ...
            net = layers.conv2d(net, 3, 128, 1, name='conv1_1', batch_normed=True)
            net = tf.nn.relu(net)

            net = layers.conv2d(net, 4, 128, 2, name='conv1_2', batch_normed=True)
            net = tf.nn.relu(net)

            net = layers.conv2d(net, 3, 256, 1, name='conv2_1', batch_normed=True)
            net = tf.nn.relu(net)
