...
from AdvGAN_2.utils import make_meshgrid, to_img
...
def scatter_samples_and_loss(x_, y_):
    ...
    def f(samples, gan=None, X=None, y=None):
        ...
        if gan.adv:
            ...
            plot_contours(ax, lambda x: np.argmax(gan.classifier.predict(x), axis=1), xx, yy)
            plt.scatter(X[:, 0], X[:, 1], color=colors[y])
            y_pred = gan.classifier.predict(samples)
...
def show_samples_and_loss(nb_channels=3):
    def f(samples, gan=None):
        ...
        fig, axarr = plt.subplots(columns, rows)
...
def plot_samples_mean_std(dim=48):
    def plot_mean_std(samples, color='blue'):
        ...
        plt.plot(mean_spectrum + std_spectrum, linestyle='dotted', label='+std')
        plt.fill_between(range(len(mean_spectrum)), mean_spectrum + std_spectrum, mean_spectrum - std_spectrum,
...
def show_cycle_samples_and_loss(nb_channels_s=3, nb_channels_t=3, nb_img=None, bands=None, size_s=None, size_t=None):
    ...
    return lambda xs, xt, fake_xt, fake_xs, reconstruct_xs, reconstruct_xt, gan: f(xs, xt, fake_xt, fake_xs,
                                                                                   reconstruct_xs, reconstruct_xt, gan)


def save_cycle_samples_and_loss(nb_channels_s=3, nb_channels_t=3, nb_img=None, path='./training_save', bands=None,
                                size_s=None, size_t=None):
    if not os.path.exists(path):
        os.makedirs(path)
    ...
    def f(xs, xt, fake_xt, fake_xs, reconstruct_xs, reconstruct_xt, gan=None):
        ...
        if (size_s is None) or (size_t is None):
            ...
            size = [size_s, size_t, size_s]
        make_fig_cycle(imgs, channels, total, columns, bands=bands, size=size,
                       path=os.path.join(path, 's2t_' + str(idx).zfill(3) + '.jpg'))
        ...
        if (size_s is None) or (size_t is None):
            ...
            size = [size_t, size_s, size_t]
        make_fig_cycle(imgs, channels, total, columns, bands=bands, size=size,
                       path=os.path.join(path, 't2s_' + str(idx).zfill(3) + '.jpg'))
        ...
        plt.close()
        plt.plot(gan.acc_t_logs)
        plt.savefig(os.path.join(path, 'accuracy_target ' + '.jpg'))
        plt.clf()
        plt.close()
...
def show_sample_and_loss_v2(nb_channels=3, bands=(0, 1, 2), size=128, total=32):
    ...
    def f(samples, gan=None):
        ...
        print('samples :')
        t_plot, ax = plt.subplots(
            nrows=rows, ncols=columns, sharex='all', sharey='none',
        )
